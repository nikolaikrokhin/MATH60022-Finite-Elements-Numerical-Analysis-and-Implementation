...
class FunctionSpace(object):
    ...
    def __init__(self, mesh, element):
        ...
        def G(d,i):
            return sum(element.nodes_per_entity[delta]*mesh.entity_counts[delta] for delta in range(d)) + i * element.nodes_per_entity[d]
        M = np.zeros((mesh.cell_vertices.shape[0],len(element.nodes)),dtype=int)
        for c in range(M.shape[0]):
            for delta in element.entity_nodes.keys():
                for eps in element.entity_nodes[delta].keys():
                    i = mesh.adjacency(mesh.dim,delta)[c,eps]
                    if delta == mesh.dim and eps == 0:
                        i = c
                    M[c,element.entity_nodes[delta][eps]] = list(range(G(delta,i),G(delta,i)+element.nodes_per_entity[delta]))
        self.cell_nodes = M
...
class Function(object):
    ...
    def integrate(self):
        ...
        for c in range(mesh.cell_vertices.shape[0]):
            ...
            integral += np.einsum("i, ji, j",self.values[fs.cell_nodes[c,:]],phi,Q.weights) * detJ
        return integral
