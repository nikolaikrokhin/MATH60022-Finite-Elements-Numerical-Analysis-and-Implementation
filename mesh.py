...
class Mesh(object):
    ...
    def __init__(self, vertex_coords, cell_vertices):
        ...
        self.gradtab = LagrangeElement(self.cell, 1).tabulate(np.array([[0]*self.dim]),grad=True)
    ...
    def adjacency(self, dim1, dim2):
        ...
        if dim2 > dim1:
            raise ValueError("""dim2 must be less than or equal to dim1.""")
        if dim2 < 0:
    ...
    def jacobian(self, c):
        ...
        J = np.einsum('ij,jik->jk',self.vertex_coords[self.cell_vertices[c,:]],self.gradtab)
        return J
